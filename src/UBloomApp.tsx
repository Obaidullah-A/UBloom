...
import {
  ...
  Sparkles, Brain, Zap, ChevronRight, Menu, Lock, Gift, Ticket, ShieldCheck, Crown
} from 'lucide-react';
...
const UBloomApp = () => {
  ...
  const fontStyle = { fontFamily: "'Rajdhani', 'Orbitron', sans-serif" };
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 flex items-center justify-center p-4" style={fontStyle}>
        <div className="max-w-md w-full bg-slate-950/90 backdrop-blur-xl rounded-3xl shadow-2xl p-10 border border-blue-800/30">
          ...
          <div className="space-y-5 mb-8">
            <input type="text" placeholder="Full Name" className="w-full p-4 bg-slate-900/50 border-b-2 border-blue-800/50 text-blue-100 placeholder-slate-600 focus:outline-none focus:border-blue-500 transition-all" />
            <input type="email" placeholder="Email" className="w-full p-4 bg-slate-900/50 border-b-2 border-blue-800/50 text-blue-100 placeholder-slate-600 focus:outline-none focus:border-blue-500 transition-all" />
        ...
        <div className="max-w-7xl mx-auto p-6 grid grid-cols-3 gap-6">
          ...
          <div className="bg-slate-950/90 backdrop-blur-xl rounded-3xl p-8 border border-blue-800/30">
            <div className="flex justify-between items-center mb-6">
              <button className="text-slate-500 hover:text-blue-400"><ChevronRight className="w-5 h-5 rotate-180" /></button>
              <span className="text-blue-100 font-bold tracking-widest">PROFILE</span>
              <button className="text-slate-500 hover:text-blue-400"><ChevronRight className="w-5 h-5" /></button>
            </div>

            <div className="text-center mb-8">
              <div className="text-7xl mb-4">{pointsToday >= 50 ? '🙂' : '😞'}</div>
              <div className="flex justify-between text-sm mb-6">
                <div><div className="text-blue-100 font-bold text-lg">{streak}</div><div className="text-slate-500">Day Streak</div></div>
                <div><div className="text-blue-100 font-bold text-lg">{coins}</div><div className="text-slate-500">Coins</div></div>
                <div><div className="text-blue-100 font-bold text-lg">{pointsToday}</div><div className="text-slate-500">Points Today</div></div>
              </div>

              {/* 50-pt progress bar */}
              <div className="mb-3">
                <div className="flex justify-between text-xs text-slate-400 mb-1">
                  <span>Daily progress</span><span>{Math.min(pointsToday,50)}/50</span>
                </div>
              ...
              {streakBroken && (
                <div className="text-xs text-amber-300 mt-3 flex items-center justify-center gap-2">
                  <Flame className="w-4 h-4" /> Streak broken
                  <button onClick={reviveStreak} className="ml-2 px-2 py-1 rounded border border-blue-700 text-blue-100 hover:bg-blue-900/30">Revive (100)</button>
                </div>
              )}
              {isPremium && insuranceUsedThisWeek && (
                <div className="text-xs text-blue-300 mt-2 flex items-center justify-center gap-2">
                  <ShieldCheck className="w-4 h-4" /> Streak insurance used this week
                </div>
              )}
            </div>

            <button onClick={() => setShowShop(true)} className="w-full mt-6 py-3 rounded-xl text-blue-100 font-bold border-2 border-blue-700 hover:bg-blue-900/30 transition-all tracking-widest">
              CUSTOMIZE
            </button>
          </div>
              ...
              <div className="space-y-3">
                {goals.slice(0,3).map(goal => (
                  <div key={goal.id} className={`flex items-center gap-4 p-4 rounded-xl border ${goal.status==='done'?'bg-blue-900/20 border-blue-800/50':'bg-slate-900/30 border-slate-800'}`}>
                    ...
                    {goal.status==='active' && (
                      <>
                        <button onClick={() => markDone(goal)} className="px-2 py-1 rounded-md border border-blue-700 text-blue-100 text-xs hover:bg-blue-900/30">Done</button>
                        <button onClick={() => markSkipped(goal)} className="px-2 py-1 rounded-md border border-slate-700 text-slate-300 text-xs hover:bg-slate-800/40">Skip</button>
                      </>
                    )}
            ...
            <div className="space-y-3 mb-8">
              ...
              {activeGoals.map(goal => (
                <div key={goal.id} className="p-4 rounded-xl bg-slate-900/30 border border-slate-800 flex items-center gap-3">
                  ...
                  <button onClick={() => markDone(goal)} className="px-3 py-2 rounded-md border border-blue-700 text-blue-100 text-xs hover:bg-blue-900/30">Done</button>
                  <button onClick={() => markSkipped(goal)} className="px-3 py-2 rounded-md border border-slate-700 text-slate-300 text-xs hover:bg-slate-800/40">Skip</button>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-3">
              ...
              {history.map(goal => (
                <div key={goal.id} className={`p-4 rounded-xl border ${goal.status==='done'?'bg-blue-900/20 border-blue-800/50':'bg-slate-900/30 border-slate-800'} flex items-center gap-3`}>
                  ...
                  {goal.status==='done' && <span className="text-xs text-blue-400 font-bold">+20</span>}
                  <button onClick={() => markActive(goal)} className="px-3 py-2 rounded-md border border-slate-700 text-slate-300 text-xs hover:bg-slate-800/40">Make Active</button>
                </div>
              ))}
            </div>
          </div>
